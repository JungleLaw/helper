...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  function recalculateAll() {
    allSkus.forEach((sku, id) => {
      // 计算日均销量 (加权平均模型)
      let avg7 = sku.sales7 > 0 ? sku.sales7 / 7 : 0;
      let avg30 = sku.sales30 > 0 ? sku.sales30 / 30 : 0;
      let dailyAvg = 0;

      if (sku.stock === 0) {
        // 【断码补偿模式】
        // 当可用库存为 0 时，近期的销量数据往往因为断货而被严重压制（真实需求远大于实际销量）
        // 此时取近期和长期的最高均值为基准，直接乘以 1.8 的激进补偿系数
        dailyAvg = Math.max(avg7, avg30) * 1.8;
      } else if (sku.sales7 >= 0 && sku.sales30 > 0) {
        if (avg7 > avg30) {
          // 【上行爆发周期】
          // 当近7日平均日销量大于30日平均日销量时，直接使用真实的近7日均值乘以 1.2作为绝对基准
          dailyAvg = avg7 * 1.2;
        } else {
          // 【平稳或下行周期】 快速响应衰减趋势防止积压
          // (将权重微调至极端的 7.5 : 2.5，兼顾猛切断流和少量平滑)
          dailyAvg = (avg7 * 0.75) + (avg30 * 0.25);
        }
      } else if (sku.sales30 > 0) {
        dailyAvg = avg30;
